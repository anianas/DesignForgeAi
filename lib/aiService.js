...
const stripCodeFences = (s) => {
  let t = (s || '').trim();
  if (t.startsWith('```')) {
    t = t.replace(/^```(?:jsx|tsx|javascript|js)?\s*/i, '').replace(/\s*```$/, '');
  }
  return t.trim();
};
...
const formatConceptForPrompt = (concept) => {
  ...
  const features = (concept.features || [])
    .map((f, i) => `  ${i + 1}. ${f.name}${f.rationale ? ' — ' + f.rationale : ''}`)
    .join('\n');
  const kpis = (concept.kpis || []).map((k) => `  • ${k.metric}`).join('\n');
  const audience = (concept.audience || []).map((a) => `  • ${a.role}`).join('\n');
  return `
